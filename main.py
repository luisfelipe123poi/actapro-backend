...
from fastapi import FastAPI, Depends, Header, HTTPException, status, UploadFile, File, Form
from typing import Optional
from pathlib import Path
import uuid

# Diccionario de límites de generación en el servidor (Seguridad del Backend)
LIMITES_GENERACION_MINUTOS_SERVER = {
    "free": 15,
    "basico": 180,
    "profesional": 300,
    "corporativo": 600
...
def validar_licencia_y_limites(email: str = Header(...)):
    """
    Dependency de FastAPI que valida en tiempo real contra MongoDB 
    el plan del usuario y sus capacidades.
    """
    if not email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Falta el correo de autenticación del usuario en los headers."
        )
    
    # Consultar base de datos real (MongoDB)
    usuario = users_collection.find_one({"email": email.lower().strip()})
    
    if not usuario:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="El usuario no se encuentra registrado en el servidor."
        )
    
    plan = usuario.get("plan", "free").lower()
    horas_restantes = float(usuario.get("horas_restantes", 0.0))
    tokens_usados = int(usuario.get("tokens_usados", 0))
    limite_tokens = int(usuario.get("limite_tokens_mes", 0))
    
    # Validar si tiene horas disponibles
    if horas_restantes <= 0 and plan != "corporativo":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Tu licencia no tiene horas de audio disponibles. Recarga tu saldo para continuar."
        )
        
    # Validar si superó tokens (si aplica)
    if limite_tokens > 0 and tokens_usados >= limite_tokens:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Has superado el límite de tokens mensuales permitidos en tu plan."
        )
        
    # Inyectamos los límites permitidos según su plan real en DB
    usuario["limite_max_minutos"] = LIMITES_GENERACION_MINUTOS_SERVER.get(plan, 15)
    return usuario

@app.get("/api/verificar-licencia")
async def verificar_licencia_endpoint(email: str):
    """Endpoint auxiliar para que el frontend consulte los datos reales del usuario."""
    usuario = users_collection.find_one({"email": email.lower().strip()})
    if not usuario:
        return {"plan": "free", "limite_generacion_minutos": 15}
    
    plan = usuario.get("plan", "free").lower()
    return {
        "plan": plan,
        "limite_generacion_minutos": LIMITES_GENERACION_MINUTOS_SERVER.get(plan, 15),
...
async def procesar_asamblea(
    ...
    # AQUí OCURRE LA MAGIA DE SEGURIDAD DEL BACKEND:
    usuario_validado: dict = Depends(validar_licencia_y_limites)
):
    try:
        # A este punto, FastAPI ya verificó en MongoDB que:
        # 1. El usuario existe.
        # 2. Su plan es legítimo y tiene saldo/horas.
        
        limite_permitido_minutos = usuario_validado["limite_max_minutos"]
        
        # Opcional: Podrías hacer una validación adicional del tamaño/duración del archivo 
        # en bytes o validando la cabecera si quisieras blindarlo al 100% también en backend.
        ...
        # Lanzar tarea asíncrona (ej: Celery)
        ...
        return {
            "status": "pending",
            "task_id": task.id,
            "message": "Archivo validado y procesamiento iniciado correctamente en el servidor."
        }
        ...
        raise HTTPException(
            ...
            detail=f"Error interno procesando la solicitud: {str(e)}"
        )
...
def reset_password_direct(email: str):
    ...
    users_collection.update_one(
        {"_id": user["_id"]},
        {"$set": {"password": password_temporal}}
    )
...
@user_config_router.post("/get-current-password")
def get_current_password(payload: ForgotPasswordRequest):
    clean_email = payload.email.strip().lower()
...
# ==========================================
# ENDPOINT 1: PUBLICAR SOLO EN DASHBOARD
# ==========================================
@user_config_router.post("/admin/publish-banner")
def publish_banner(payload: SystemNotificationRequest):
    # Guarda únicamente el aviso en MongoDB para que los dashboards lo muestren en tiempo real
    notification_data = {
        "title": payload.title,
    ...
    return {
        "success": True,
        "message": "Aviso publicado exitosamente en los dashboards de los clientes."
    }

# ==========================================
# ENDPOINT 2: ENVIAR SOLO CORREOS MASIVOS
# ==========================================
@user_config_router.post("/admin/send-emails-only")
def send_emails_only(payload: SystemNotificationRequest, background_tasks: BackgroundTasks):
    # Lanza únicamente la tarea en segundo plano de Brevo (1 correo por minuto) sin alterar el banner
    background_tasks.add_task(enviar_correos_pausados_brevo, payload.title, payload.message)
    
    return {
        "success": True,
        "message": "El envío dosificado de correos ha comenzado en segundo plano (1 por minuto)."
    }

# ==========================================
# ENDPOINT DE CONSULTA PARA EL DASHBOARD
# ==========================================
